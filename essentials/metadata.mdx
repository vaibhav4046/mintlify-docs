...
| Value size | 32,767 bytes (JSON-encoded, per top-level value) | `metadata value at "..." is N bytes, exceeding the maximum of 32767` |
...
These limits apply to both the `metadata` and `additional_metadata` fields when you add memories or upload knowledge. For app knowledge, the same checks run on `tenant_metadata` and `document_metadata`; `app_metadata` is not validated. If metadata is invalid, the request returns `400 INVALID_INPUT` and nothing is enqueued for processing.
