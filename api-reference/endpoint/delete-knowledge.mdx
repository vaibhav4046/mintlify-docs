...
<Info>
**Cleanup failures are retryable.** Deleting a source also clears its embeddings, graph entities, stored files, and indexing status. If that downstream cleanup fails, the source is left intact and the request returns `success: false`, `deleted_count: 0`, and `message: "Source deletion cleanup failed; retry deletion"`, with the same error on each affected entry in `results[]`. Because deletion is idempotent, re-sending the same request finishes any remaining cleanup and succeeds once everything is gone.
</Info>
