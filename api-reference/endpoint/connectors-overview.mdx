...
| [`/v2/connectors/:id/discover`](/api-reference/endpoint/discover-channels) | `GET` | Discover available resources (Slack channels, GitHub repos, Notion databases, Linear teams/projects) | No |
| [`/v2/connectors/:id/configure`](/api-reference/endpoint/configure-connector) | `POST` | Bulk-add resources with shared settings | No |
...
**Provider** – The external application type, such as `slack`, `notion`, `github`, or `linear`. Each provider has specific authentication requirements and resource types.
...
|---|---|---|
| `api_token` | Single token authentication | Slack bot tokens (`xoxb-...`), GitHub PATs (`ghp_...`), Notion integration tokens, Linear API keys (`lin_api_...`) |
...
| `linear` | Teams and projects | Issues |
