...
<Info>
**Complete and immediate.** Deletion removes the metadata, the underlying storage, and the indexing-status record. Deleted memories cannot be fetched by their `source_id` – requests return `404 MEMORY_NOT_FOUND` immediately, and a status query for a deleted memory returns `errored`, the same as any unknown ID. This ensures compliance with right-to-deletion requirements.
</Info>
