...
### 404 vs 500 on tenant lookup

When a request fails during tenant resolution:

- **404** (`TENANT_NOT_FOUND`) means the tenant does not exist. Do not retry; verify the `tenant_id` or create the tenant first.
- **500** (`INTERNAL_ERROR`) means the lookup itself failed (for example, a transient database timeout). Retry with exponential backoff.
